...
create table if not exists public.faculty_salary_rows (
  id bigint generated by default as identity primary key,
  file_name text not null,
  file_url text not null,
  period_year integer not null,
...
alter table public.faculty_salary_rows enable row level security;

drop policy if exists "Allow program users to read faculty salary rows" on public.faculty_salary_rows;
drop policy if exists "Allow program users to insert faculty salary rows" on public.faculty_salary_rows;

create policy "Allow program users to read faculty salary rows"
  on public.faculty_salary_rows for select
  to anon, authenticated using (true);

create policy "Allow program users to insert faculty salary rows"
  on public.faculty_salary_rows for insert
  to anon, authenticated with check (true);
