...
const startQrScanner = async () => {
  ...
  const currentMovement = letter.movements.find(
    (movement) =>
      movement.status === "in_progress"
  );
  ...
  if (!currentMovement.received_at) {
    alert(
      "يجب تسجيل استلام الخطاب أولًا قبل تسليمه."
    );
    return;
  }

  const nextMovement = letter.movements.find(
    (movement) =>
      movement.step_order >
        currentMovement.step_order &&
      movement.status === "waiting"
  );
  ...
  try {
    ...
    alert(
      nextMovement
        ? "تم تسليم الخطاب. المحطة التالية في انتظار الاستلام."
        : "تم تسليم الخطاب وإغلاق حركته بنجاح."
    );
  } catch (error) {
    console.error(
      "Error delivering letter:",
      error
    );

    alert(
      "حدث خطأ أثناء تسجيل تسليم الخطاب."
    );
  }
};
 useEffect(() => {
  ...
  if (!targetMovement) {
    alert(
      "لا توجد محطة تنتظر استلام الخطاب."
    );
    return;
  }
  ...
  try {
    ...
    setScannedLetter(updatedLetter);

    await loadLetters();

    alert(
      "تم تسجيل استلام الخطاب بنجاح."
    );
  } catch (error) {
    console.error(
      "Error receiving letter:",
      error
    );

    alert(
      "حدث خطأ أثناء تسجيل استلام الخطاب."
    );
  } finally {
    setReceiving(false);
  }
...
const confirmLetterDelivered = async () => {
  ...
  if (!currentMovement) {
    alert(
      "لا توجد محطة حالية لتسليم الخطاب."
    );
    return;
  }
